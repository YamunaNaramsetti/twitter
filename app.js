...
const getFollowingPeopleIdsOfUser = async (username) => {
  ...
  const arrayOfKids = followingPeople.map(
...
app.post("/register/", async (request, response) => {
  ...
  if (userDbDetails !== undefined) {
    ...
    if (password.length < 6) {
      ...
      const createUserQuery = `INSERT INTO user(name,password,gender,username)
                VALUES('${name}',
               '${password}',
               '${gender}',
               '${username}'
...
app.get("/user/followers/", authentication, async (request, response) => {
  ...
  const getFollowersQuery = `SELECT 
        DISTINCT name FROM follower INNER JOIN user ON user.user_id=follower.follower_user_id
